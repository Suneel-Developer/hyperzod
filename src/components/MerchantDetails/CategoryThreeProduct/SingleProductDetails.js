...
import SubProductImage1 from "../../../assets/fastfood-category.jpeg";
import SubProductImage2 from "../../../assets/sub-product2.png";
import VegetableProductOne from "../../../assets/lettuce.jpeg";
import VegetableProductTwo from "../../../assets/tomatoes.jpeg";
import VegetableProductThree from "../../../assets/olives.jpeg";
import VegetableProductFour from "../../../assets/avocado.jpeg";
...
const SingleProductDetails = ({ onClose }) => {
    ...
    const handleVegetableToggle = (vegetable) => {
        setSelectedVegetables((prev) =>
            prev.includes(vegetable) ? prev.filter((item) => item !== vegetable) : [...prev, vegetable]
        );
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
            <div className="bg-white rounded max-w-[1200px] h-screen lg:overflow-hidden overflow-y-auto lg:h-[600px] mx-auto w-full overflow-hidden animate-slide-up grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className='relative'>
                    ...
                    <div className='h-auto lg:h-[600px] overflow-y-auto boxscrollbar pt-10 pb-20'>
                        <div className='px-4'>
                            <h4 className="font-medium text-sm">FastFood</h4>
                            <h2 className="font-semibold text-xl leading-7">Baja Turkey with Sliced Avocado</h2>
                            <small className="text-[#A3A3A3] text-sm leading-4 line-through mt-1">£14.00</small>
                            <p className="text-sm leading-5 mb-1 text-[#9CA3AF]">Spice up your salad with Oven-Roasted Turkey, and Sliced Avocado, on a bed of greens and crisp veggies and topped with Baja Chipotle sauce.</p>
                        </div>

                        {/* Select options  */}
                        <div className='pt-10'>
                            <div className='w-full bg-[#f2f2f2] px-4 py-2 text-sm font-semibold'>Cheese</div>
                            <ul className='py-1'>
                                {['American Cheese', 'Cheddar Cheese', 'Swiss Cheese'].map((cheese) => (
                                    <li
                                        key={cheese}
                                        className={`py-3 px-4 flex items-center gap-4 cursor-pointer transition-all duration-500 hover:bg-[#f2f2f2d2]`}
                                        onClick={() => handleCheeseSelect(cheese)}
                                    >
                                        <input
                                            type="radio"
                                            id='specifyColor'
                                            className={`w-4 h-4 cursor-pointer opacity-70 ${selectedCheese === cheese ? 'opacity-100' : ''}`}
                                            checked={selectedCheese === cheese}
                                            readOnly
                                        />
                                        <p className='text-[15px] flex-1 truncate'>{cheese}</p>
                                        <small className='text-xs font-semibold'>£2.00</small>
                                    </li>
                                ))}
                            </ul>

                            <div className='w-full bg-[#f2f2f2] px-4 py-2 text-sm font-semibold'>Add Ons</div>
                            <ul className='py-1'>
                                {['Bacon', 'Pepperoni', 'Extra Cheese', 'Fresh Mozzarella'].map((addOn) => (
                                    <li
                                        key={addOn}
                                        className={`py-3 px-4 flex items-center gap-4 cursor-pointer transition-all duration-500 hover:bg-[#f2f2f2d2]`}
                                    ...
                                    >
                                        <input
                                            type="checkbox"
                                            id='specifyColor'
                                            className={`w-4 h-4 cursor-pointer opacity-70 ${selectedAddOns.includes(addOn) ? 'opacity-100' : ''}`}
                                            checked={selectedAddOns.includes(addOn)}
                                            readOnly
                                        />
                                        <p className='text-[15px] flex-1 truncate capitalize'>{addOn}</p>
                                        <small className='text-xs font-semibold'>£{addOn === 'Pepperoni' ? '3.00' : '2.00'}</small>
                                    </li>
                                ))}
                            </ul>

                            <div className='w-full bg-[#f2f2f2] px-4 py-2 text-sm font-semibold capitalize'>Vegetable</div>
                            <ul className='py-1'>
                                {[['Lettuce', VegetableProductOne], ['Tomatoes', VegetableProductTwo], ['Olives', VegetableProductThree], ['Avocado', VegetableProductFour]].map(([vegetable, image]) => (
                                    <li
                                        key={vegetable}
                                        className={`py-3 px-4 flex items-center gap-4 cursor-pointer transition-all duration-500 hover:bg-[#f2f2f2d2]`}
                                    ...
                                    >
                                        <input
                                            type="checkbox"
                                            id='specifyColor'
                                            className={`w-4 h-4 cursor-pointer opacity-70 ${selectedVegetables.includes(vegetable) ? 'opacity-100' : ''}`}
                                            checked={selectedVegetables.includes(vegetable)}
                                            readOnly
                                        />
                                        <p className='text-[15px] flex-1 truncate capitalize'>{vegetable}</p>
                                        <img src={image} alt={vegetable} className='md:h-20 h-14 rounded object-cover' />
                                    </li>
                                ))}
                            </ul>
